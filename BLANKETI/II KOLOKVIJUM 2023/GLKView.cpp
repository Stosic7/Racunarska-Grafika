...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar) {
	case VK_LEFT:
		m_glRenderer.RotateView(-5.0, 0);
		break;
	case VK_RIGHT:
		m_glRenderer.RotateView(5.0, 0);
		break;
	case VK_UP:
		m_glRenderer.RotateView(0, -5.0);
		break;
	case VK_DOWN:
		m_glRenderer.RotateView(0, 5.0);
		break;
	case VK_OEM_MINUS:
		m_glRenderer.ZoomView(0.1);
		break;
	case VK_OEM_PLUS:
		m_glRenderer.ZoomView(-0.1);
		break;
	case '1':
		m_glRenderer.m_rot1 += 5;
		if (m_glRenderer.m_rot1 > 90)
			m_glRenderer.m_rot1 = 90;
		break;
	case '2':
		m_glRenderer.m_rot1 -= 5;
		if (m_glRenderer.m_rot1 < 0)
			m_glRenderer.m_rot1 = 0;
		break;
	case '3':
		m_glRenderer.m_rot2 += 5;
		if (m_glRenderer.m_rot2 > 90)
			m_glRenderer.m_rot2 = 90;
		break;
	case '4':
		m_glRenderer.m_rot2 -= 5;
		if (m_glRenderer.m_rot2 < -90)
		...
		break;
	case '5':
		m_glRenderer.m_rot3 += 5;
		if (m_glRenderer.m_rot3 > 90)
			m_glRenderer.m_rot3 = 90;
		break;
	case '6':
		m_glRenderer.m_rot3 -= 5;
		if (m_glRenderer.m_rot3 < -90)
