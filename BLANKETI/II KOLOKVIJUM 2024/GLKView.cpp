...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	switch (nChar) {
	case VK_RIGHT:
		m_glRenderer.RotateView(5.0, 0.0);
		break;
	case VK_LEFT:
		m_glRenderer.RotateView(-5.0, 0.0);
		break;
	case VK_UP:
		m_glRenderer.RotateView(0.0, 5.0);
		break;
	case VK_DOWN:
		m_glRenderer.RotateView(0.0, -5.0);
		break;
	case VK_OEM_PLUS:
		m_glRenderer.ZoomView(-1.0);
		break;
	case VK_OEM_MINUS:
		m_glRenderer.ZoomView(1.0);
		break;
	case 'A':
		m_glRenderer.m_flowerAngle += 5.0;
		if (m_glRenderer.m_flowerAngle > 90.0) m_glRenderer.m_flowerAngle = 90.0;
		break;
	case 'S':
		m_glRenderer.m_flowerAngle -= 5.0;
		if (m_glRenderer.m_flowerAngle < 0.0) m_glRenderer.m_flowerAngle = 0.0;
		break;
	}
